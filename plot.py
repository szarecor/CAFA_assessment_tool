        ...
        with open(result_file) as f:
            for line in f:
                if line.startswith(onto):
                    if (
                        line.split("|")[0].split()[1] == Type
                        and line.split("|")[0].split()[2] == mode
                    ):
...
def curveSmooth(result):
    ...
    for i in range(len(result.precision)):
        ...
        for j in range(i):
            if (
                result.precision[i] < result.precision[j]
                and result.recall[i] < result.recall[j]
            ):
                remove = True
                break
        if not remove:
...
def plotMultiple(title, listofResults, smooth):
    ...
    for j, i in enumerate(listofResults):
        ...
        if smooth == "Y":
            ...
            ax.plot(
                i.recall[int(i.thres * 100)],
                i.precision[int(i.thres * 100)],
                "o",
                color=colors[j],
            )
        elif smooth == "N":
        ...
        for onto in ["bpo", "cco", "mfo"]:
            for Type in ["LK", "NK"]:
                for mode in ["partial", "full"]:
                    ...
                    for method in methods:
                        res = getprrc(
                            onto, Type, mode, prrcfolder, method, results_folder
                        )
